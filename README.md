...
Reward shape (on top of the upstream `unitree_rl_lab` Go2 velocity env): yaw-tracking disabled (forward-only task), Gaussian `track_lin_vel_xy` as the primary tracking signal (weight 0.75), soft penalty rebalance for fast gaits, and gait shaping (`feet_air_time`, `feet_gait`, `feet_clearance`). A small linear partial-credit term `track_lin_vel_x_linear` is kept at weight 0.15 — low enough that matching the command is ~9× more rewarding than cruising below it, but high enough to give a random initial policy gradient signal for forward motion and bootstrap walking.
